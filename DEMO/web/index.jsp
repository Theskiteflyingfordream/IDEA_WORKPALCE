...
<%--不用ajax之前的做法，index转发给controller，再到list，list中的分页使用超链接到controller再回到list--%>
...
<script type="text/javascript">
    /*全局变量，当前页面的总记录数*/
    var totalPages;
    /*工具方法，发送ajax请求访问服务端获取第pn页的数据并显示*/
    function to_page(pn){
        $.ajax({
            url:"${APP_PATH}/emps",
            data:"pn="+pn,
    ...
    function validate_email(){
        ...
        if (regEmail.test(email)) {
            ...
            show_validate_msg($("#email_add_input"), "fail", "邮件名必须符合规范");
            return false;
        }
    }

    /*工具方法：对模态框的填写处添加样式以及提示信息*/
    ...
    /*点击新增按钮弹出模态框。这个script放在head中点击没有响应？*/
    ...
    /*名字内容变化就发送ajax请求进行前端校验，判断用户名与数据库中的是否重复*/
    ...
    /*点击编辑按钮弹出模态框，注意编辑按钮是在页面加载完成后创建的，使用.click绑定不了*/
    $(document).on("click",".edit_btn",function(){
        //清除表单数据（表单完整重置（表单的数据，表单的样式））
        reset_form("#empUpdateModal form");
        //获取部门信息显示
        getDepts("#empUpdateModal select");
        //ajax获取原来的员工信息并显示到修改框中
        $.ajax({
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                if(result.code==200){
                    if (undefined != result.extend.errorFields.email) {
                        //显示邮箱错误信息
                        show_validate_msg($("#email_update_input"), "fail", result.extend.errorFields.email);
                        return false;
                    }
                }else{
                    //1、关闭对话框
